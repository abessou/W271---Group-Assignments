...
Note that this answer assumes that the question requests an answer for the data scientist being an expert in machine learning OR statistics and not exclusively an expert in machine learning or  scientistics.
...
$$E[\widehat{\sigma^{2}}] = \frac{1}{n} \sum_{i=1}^{n} [E(Y_{i}^2) - \frac{2}{n} E(Y_{i} \sum_{j=1}^{n} \frac{Y_{i}}{n}) + E(\bar{Y}^{2})]$$
$$E[\widehat{\sigma^{2}}] = \frac{1}{n} \sum_{i=1}^{n} [E(Y_{i}^2) - \frac{2}{n} E(Y_{i} \sum_{j =0, j \neq i}^{n} Y_{j} + Y_{i} Y_{i}) + E(\bar{Y}^{2})]$$
...
Going back to $E[\widehat{\sigma^{2}}]$, we use the iid property to reduce $E(Y_{i} \sum_{j =0, j \neq i}^{n} Y_{j}) + E(Y_{i}^2)$ since $Y_{i}$ and $Y_{j}$ are identical and independent varaibles. Thus:
$$E(Y_{i} \sum_{j =0, j \neq i}^{n} Y_{j}) = \sum_{j=0, j \neq i}^{n} E(Y_{i}Y_{j}) = \sum_{j=0, j \neq i}^{n} E(Y_{i})E(Y_{j}) = (n-1)\mu^{2}$$

...
observe that after the first difference is taken, the ACF of the series, just like the PACF do not show noticeble dynamic in the mean. We have no reason to suspect an additional MA component in the original series. But the first differential series does show clustered volatility in the variance.
...
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=60)}
data.best <- get.best.sarima(data$DXCM, maxord=rep(3,6))
data.best$best

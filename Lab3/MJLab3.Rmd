...
8. __Try SARIMA models.__  Use the get.best.sarima() function.  The best AIC output is -90.17105 with parameters c(0,1,1,1,0,1), but for parsimony we will choose a SARIMA(0,1,0,0,0,1) with an AIC of -76.45286 which is very close to the other model.  Check the residuals.  Yes, the residuals look basically like white noise.  There is one place in the squared residuals where the value exceeds the 95% confidence interval.  The residuals plot exhibits evidence of time-varying volatility.  The In-Sample fit of this estimated model.

9. __Backtesting.__ MJ WILL FINISH WRITING THIS LATER.  Backtesting looks good with all values in the confidence intervals.

10. __Forecast the model.__  MJ WILL FINISH WRITING LATER.  Using the SARIMA model, we will make the requested 12-step ahead forecast of the model.  The forecast looks like it captures the seasonality of the model as it matches the upward trend and the seasonal volatility.
...
# Plot the time series
plot.time.series(glob.warm.part.ts,50, "GW 2013-2016")
...
# Plot residuals
par(mfrow=c(1,1))
hist(glob.warm.part.ar1$residuals,30)
plot.residuals.ts(glob.warm.part.ar1, "AR(1)")
par(mfrow=c(2,1))
# Plot the In-sample fit
plot.orig.model.resid(glob.warm.part.ts, glob.warm.part.ar1, "AR(1)", c(2013,2016), c(-0.7,4))
...
# Plot residuals
par(mfrow=c(1,1))
hist(glob.warm.part.arima$residuals,30)
plot.residuals.ts(glob.warm.part.arima, "AR(0,1,0)")
# Plot In-sample fit
plot.orig.model.resid(glob.warm.part.ts, glob.warm.part.arima, "AR(0,1,0)", c(2013,2016), c(-0.7,4))
...
# Plot the residuals
par(mfrow=c(1,1))
hist(glob.warm.part.sarima$residuals,30)
plot.residuals.ts(glob.warm.part.sarima, "SARIMA(0,1,0,0,0,1)")
par(mfrow=c(2,1))
acf(glob.warm.part.sarima$residuals, main="GW 2013-2016 SARIMA(0,1,0,0,0,1) Residuals")
acf(glob.warm.part.sarima$residuals^2, main="GW 2013-2016 SARIMA(0,1,0,0,0,1) Residuals")
# Plot the In-sample fit
plot.orig.model.resid(glob.warm.part.ts, glob.warm.part.sarima, "SARIMA(0,1,0,0,0,1)", c(2014,2016.5), c(-0.7,5))
...
head(df.part)
# Plot the original and estimate series with residuals
par(mfrow = c(1, 1))
plot.ts(df.part[, "orig_series"], col = "red", main = "Original vs a SARIMA Estimated Series with Residuals",
        ylab = "Original and Estimated Values", xlim = c(2013, 2016),
        ylim = c(-.7,4))
par(new = T)
...
par(mfrow = c(1, 1))
plot(glob.warm.part.sarima.bt.fcast, lty = 2, col = "navy", main = "Out-of-Sample Forecast",
...
plot.model.forecast(glob.warm.part.sarima, glob.warm.part.sarima.fcast,
                    "12", c(2013,2017.5), c(-0.7,5.5))

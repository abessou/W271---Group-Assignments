...
$$MLE(\theta) = \hat{\theta} = max(y_{1},\cdots,y_{n})$$
...
- __wage__ - The mdMiss participants have lower wages than the mdHave participants.  The minimum value for wage for mdMiss is $127 vs. $136 for mdHave.  The median and mean for values for mdMiss are lower at $481 vs. $570 and $531 vs. $597, respectively.  The maximum values are also lower at $2,083 vs. $2,404.\newline
- __education__ - The mdMiss participants have less education than the mdHave participants.  This could indicate that they stopped education sooner and went to work earlier than the mdHave participants.  The mean value for mdMiss is 12.09 vs. 13.65 for mdHave, a difference of 1.56 years or a 11.4% decrease.\newline
- __experience__ - The mdMiss participants have more experience than the mdHave participants.  This is further evidence that they stopped education sooner and went to work earlier than the mdHave participants. The mean value for mdMiss is 10.47 vs. 8.145 for mdHave, a difference of 2.32 years or a 28.5% increase.   \newline
- __raceColor__ - The mdMiss participants have a much higher percentage of people with the raceColor variable set to 1 than mdHave.  44.77% (124 people) with 1's for mdMiss vs. 15.77% (114 people) with 1's for mdHave.\newline
...
# Create 2 temporary datasets.  mdMiss contains all of the rows with either mom_education
# or dad_education equal to NA.  mdHave contains all of the rows with both mom_education and
# dad_education equal to a non-NA value.
...
# Use summary to confirm that mdMiss has the correct number of NA's for mom_education and
# dad_education
summary(mdMiss)
summary(mdHave)
# Use str to confirm that the datasets have the appropriate number of observations
str(mdMiss)
str(mdHave)

# Compare the summaries of the mdMiss and mdHave datasets to see the differences in 
# wage, eduction, experience and raceColor
summary(mdMiss$wage)
summary(mdHave$wage)
...
OLS.logWage.6var = lm(logWage ~ education + experience + experienceSquare + raceColor + rural + city, data=data)
...
# Run the IV TSLS regression with z1 and z2 with only 6 variables, removing mom_education
# and dad_education.
TSLS.logWage.6var = ivreg(logWage ~ education + experience + experienceSquare + raceColor + rural + city | z1*z2 + experience + experienceSquare + raceColor + rural + city, data=data)

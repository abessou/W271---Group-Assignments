...
$$\mathbf{MLE(\theta) = \hat{\theta_{ml}} = max(y_{1},\cdots,y_{n})}$$

Part 3 - Expectation n=1
------------------------
Taking $\hat{\theta_{ml}} = max(y_{1},\cdots,n)$ and n=1\newline
We have
$$\hat{\theta_{ml}} = y_{1}$$
And
$$\mathbf{E[\hat{\theta_{ml}}] =  E[y_{1}] = \frac{\theta}{2}}$$
Knowing that $y_{i}$ is from a random uniform distribution over $[0,\theta]$

Part 4 - Bias
-------------
Yes, from the above, $\hat{\theta_{ml}}$ is biased. For any ${y_{1},\cdots, y_{n}}$, we expect $max(y_{1},\cdots,y_{n}) < \theta$ with probability 1. Hence $\hat{\theta_{ml}}$ underestimates $\theta$ and we have just proven that for n=1, $E(\hat{\theta_{ml}}) \neq \theta$.

Part 5 - Expectation general case
---------------------------------
Taking $\hat{\theta_{ml}}=max(y_{1},\cdots,y_{n})$ and assuming $n \geq 1$.
$$E(\hat{\theta_{ml}}) = E[max(y_{1},\cdots,y_{n})]$$
...
$$\mathbf{\lim_{n \rightarrow \infty} \hat{\theta_{ml}} = \theta}$$
...
__Assumption 1:__  Effects of any omitted variables are constant over time.  The error terms that are endogenous, are also time constant, and would therefore cancel out.\newline
__Assumption 2:__ Exogeneity of the variables in the model as formulated as:\newline
...
```{r, tidy=TRUE, tidy.opts=list(width.cutoff=70)}
rs_train = subset(retailSales, retailSales$Year == 2004 | retailSales$Year == 2005)
m1 = lm(Revenue~Planned.revenue, data=rs_train)
summary(m1)
(.9694 - .95) / (0.0003921 / sqrt(14085))
```
...
1. Cov(z1, Planned.Revenue) != 0. z1 needs to have some correlation with the variable, Planned.Revenue, for which it is an
instrument.  This could be a reasonable assumption because stock price is a historical indicator of a company's finanical picture which could potentially be related to Planned.Revenue.\newline
